...
class Agent:
    ...
    async def execute_tool_call(self, tool_name: str, arguments: Dict[str, str]) -> Any:
        """
        ...
        try:
            if isinstance(arguments, str):
                selectedTool = self.toolbox.tools[tool_name]
                print("***selectedTool: " + tool_name)
                observation = await selectedTool(arguments)
            else:
...
class ReactAgent(Agent):
    ...
    async def provide_final_answer(self, task) -> str:
        """
    ...
    async def run(self, task: str, stream: bool = False, reset: bool = True, **kwargs):
        """
        Runs the agent for the given task.
        ...
        if stream:
            ...
            return  await self.direct_run(task)
    ...
    async def direct_run(self, task: str):
        """
        ...
        while final_answer is None and iteration < self.max_iterations:
            try:
                if self.planning_interval is not None and iteration % self.planning_interval == 0:
                    await self.planning_step(task, is_first_step=(iteration == 0), iteration=iteration)
                step_logs = await self.step()
                if "final_answer" in step_logs:
                    final_answer = step_logs["final_answer"]
            except AgentError as e:
                self.logger.error(e, exc_info=1)
        ...
        if final_answer is None and iteration == self.max_iterations:
            ...
            self.logs.append(final_step_log)
            self.logger.error(error_message, exc_info=1)
            final_answer = await self.provide_final_answer(task)
            final_step_log["final_answer"] = final_answer

        return final_answer

    async def planning_step(self, task, is_first_step: bool = False, iteration: int = None):
        """
...
class ReactJsonAgent(ReactAgent):
    ...
    async def step(self):
        ...
        if tool_name == "final_answer":
            ...
            observation = await self.execute_tool_call(tool_name, arguments)
